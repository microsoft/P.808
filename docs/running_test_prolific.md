...
Following steps explain how to conduct the Speech Quality Assessment test on Prolific according to
...
As a result you should have a directory named YOUR_PROJECT_NAME which contains:
   ...
   * `YOUR_PROJECT_NAME_ccr.html`: Customized HIT app to be used in AMT (in this example for CCR test method).
...
In paraller you need to use a system to host your survey. One option is [HITAppServer](https://github.com/microsoft/P.910/tree/main/hitapp_server). The following procedure is customized to the HIT APP server and assume a running server is available.
