...
def validate_inputs(cfg, df, method):
    ...
    # check optionals
    #   gold_clips
    if method in ['acr', 'p835', 'p831s7'] and 'number_of_gold_clips_per_session' in cfg and int(cfg['number_of_gold_clips_per_session'])>0:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Create input.csv for ACR, DCR, CCR, P835, P831s7 test. ')
    ...
    assert exp_method in methods, f"{exp_method} is not a supported method, select from: acr, dcr, ccr, p835, p831s7."
